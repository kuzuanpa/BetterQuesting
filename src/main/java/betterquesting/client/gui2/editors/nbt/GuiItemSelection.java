...
public class GuiItemSelection extends GuiScreenCanvas implements IPEventListener, IVolatileScreen {
    ...
    public void initPanel() {
        ...
        CanvasItemDatabase cvDatabase = new CanvasItemDatabase(
        ...
        itemPreview = new PanelItemSlot(
            new GuiTransform(GuiAlign.TOP_LEFT, 0, 16, 36, 36, 0),
            99,
            itemStack,
            false,
            true);
        ...
        fieldSize = new PanelTextField<>(
            new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(52, 16, 0, -32), 0),
        ...
        if (itemStack != null && !StringUtils.isNullOrEmpty(itemStack.getOreDict())) {
            ...
            for (int i = 0; i < oreIds.length; i++) {
                if (OreDictionary.getOreName(oreIds[i])
                    .equalsIgnoreCase(oreName)) {
                    oreIdx = i;
                    break;
                }
            }
        ...
        btnOre = new PanelButtonStorage<>(
            new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(52, 36, 24, -52), 0),
            2,
            "Ore: " + oreName,
            oreIdx);
        ...
        btnWild.setClickAction((b) -> {
            if (itemStack != null) {
                itemStack.getBaseStack()
                    .setItemDamage(OreDictionary.WILDCARD_VALUE);
        ...
        PanelTextBox txInvo = new PanelTextBox(
            new GuiTransform(GuiAlign.TOP_EDGE, new GuiPadding(0, 0, 0, -16), 0),
        ...
        float iScale = Math.min(
            cvBottomLeft.getTransform()
                .getWidth() / 162F,
            (cvBottomLeft.getTransform()
                .getHeight() - 20) / 72F);
        ...
        for (int i = 0; i < 27; i++) // Main inventory
        {
            ...
            cvBottomLeft.addPanel(
                new PanelItemSlot(
                    new GuiTransform(GuiAlign.TOP_LEFT, x, y, slotSize, slotSize, 0),
                    1,
                    invoStack,
                    true));
        ...
        for (int i = 0; i < 9; i++) // Hotbar
        {
            ...
            cvBottomLeft.addPanel(
                new PanelItemSlot(
                    new GuiTransform(GuiAlign.TOP_LEFT, x, 20 + (3 * slotSize), slotSize, slotSize, 0),
                    1,
                    invoStack,
                    true));
        }
    ...
    private void onButtonPress(PEventButton event) {
        ...
        if (btn.getButtonID() == 0) // Exit
        {
            ...
            if (idx >= oreIds.length || idx < 0) {
                ...
                ((PanelButtonStorage<Integer>) btn).setStoredValue(-1)
                    .setText("Ore: NONE");
                itemPreview.setStoredValue(itemStack); // Refreshes OD
            } else {
                itemStack.setOreDict(OreDictionary.getOreName(oreIds[idx]));
                ((PanelButtonStorage<Integer>) btn).setStoredValue(idx)
                    .setText("Ore: " + itemStack.getOreDict());
                itemPreview.setStoredValue(itemStack); // Refreshes OD
            }
        }
    }
